...
INSERT INTO sklad_produktow (id_produktu, skladnik) VALUES (10, 'prazony ryz');
...
DECLARE kwota_z DOUBLE PRECISION;
...
BEGIN
	...
	INSERT 
		...
		VALUES(klient, CURRENT_DATE, adres) 
	...
	INSERT
		INTO zamowione_produkty(id_zamowienia, id_produktu, ilosc, cena)
		VALUES(identit, krotka.id_produktu, krotka.ilosc, krotka.cena);
		
	kwota_z = kwota_z + (krotka.ilosc * krotka.cena);

	END LOOP;

DELETE FROM koszyki_klientow WHERE(id_uzytkownika = klient);
UPDATE zamowienia SET kwota = kwota_z WHERE id_zamowienia = identit;

END;
$$ LANGUAGE 'plpgsql';

CREATE VIEW najpopularniejsze AS

	SELECT T1.id_produktu, nazwa_produktu, kategoria, kraj, cena_za_100g, suma

	FROM
		(SELECT DISTINCT id_produktu, sum(ilosc) AS suma
		FROM zamowione_produkty 
		GROUP BY id_produktu) as T1

		JOIN 

		produkty ON (T1.id_produktu = produkty.id_produktu)
